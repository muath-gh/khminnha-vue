 ...
 <transition-group name="stagger-slide" tag="div" class="options-grid-modern">
  ...
  >
    <div class="option-card-inner">
      ...
      <div v-if="showCategories && currentStep === 'categories'" class="categories-grid" :class="{ 'has-sidebar': state.conversation.length > 0 && !isLoadingQuestions }">
      ...
      <div v-if="currentStep === 'subCategories'" class="categories-grid" :class="{ 'has-sidebar': state.conversation.length > 0 && !isLoadingQuestions }">
  <h3 class="section-title">اختر تصنيف فرعي 🔍</h3>

        <transition-group name="category-stagger" tag="div" class="categories-wrapper">
          <div
           v-for="(sub,index) in state.subCategories"
            :key="sub.id"
            class="category-card"
            :class="{ used: sub.used }"
            :style="{ '--card-index': index }"
          ...
          >
            ...
            <div class="category-image-container">
              <div class="category-image" :style="{
                backgroundImage: `url(${sub.image || getCategoryPlaceholderImage(index)})`
              }">
                <div class="category-image-overlay"></div>
              </div>
            </div>
...
async function initializeGame() {
  ...
  const theme = await gameStore.fetchThemeById(Number(themeId));
  const level = await gameStore.fetchLevelById(Number(levelId));
  const user  = await gameStore.fetchUserById(Number(userId));
